...
function PlayerTalentFrame_UpdateTabs(playerLevel)
	...
	for i = 1, MAX_TALENT_TABS do
		...
		if ( tab ) then -- If the tab exists, lets display stuff about this
			if ( i <= numTabs ) then -- Double check that the tab has info we should be displaying
...
--[[			elseif(i <=numTabs+3) then
				local name, icon, pointsSpent, background, previewPointsSpent = GetTalentTabInfo(i-numTabs, PlayerTalentFrame.inspect, PlayerTalentFrame.pet, PlayerTalentFrame.talentGroup);
				if ( i == selectedTab ) then
					-- If tab is the selected tab set the points spent info
					local displayPointsSpent = pointsSpent + previewPointsSpent;
					PlayerTalentFrameSpentPointsText:SetFormattedText(MASTERY_POINTS_SPENT, name, HIGHLIGHT_FONT_COLOR_CODE..displayPointsSpent..FONT_COLOR_CODE_CLOSE);
...
--]]
	...
	-- Begin experimental redisplay first three 
...
--]]
	local maxTotalWidth = 360;
	for i=1,MAX_TALENT_TABS do
		talentTabWidthCache[i] = 100;
		tab = _G["PlayerTalentFrameTab"..i];
		PanelTemplates_TabResize(tab,0,maxTotalWidth/3);
	end
	

	-- update the tabs
	PanelTemplates_SetNumTabs(PlayerTalentFrame, numTabs + 3 + numGlyphTabs);
